...
"""
def get_spikes(pathtofolder, binaryfile=True, input_neurons=False):
    spike_ids = list()
    spike_times = list()
    #id_filename = 'Neurons_SpikeIDs_Untrained_Epoch0'
    id_filename = 'SpikeIDs'
    #times_filename = 'Neurons_SpikeTimes_Untrained_Epoch0'
    times_filename = 'SpikeTimes'
    if (input_neurons):
...
def splitstimuli(neuronids, neurontimes, stimduration):
    ...
    #create a TF mask with which to split by duration ( ie. by stimulus)
    ...
    for s in range(len(ids)):
        ...
        if (full_stimulus):
            ...
            for i in range(neuronend - neuronstart):
                ...
                ###
                # tempCounter = 0;
                #for j in range(len(idswindow)):
                    #if (idswindow[j] == (neuronstart + i)):
                        # tempCounter = tempCounter + times[idswindow[j]]    
                # rates[i] = tempCounter / timelength
                ###
                # Possible solutions: 
                # (i)  e.g. indices = [i for i, x in enumerate(my_list) if x == "whatever"]
                # (ii) update the for loop <= S2
